...
public class Json2Consumer {
    ...
    private void printPayloadIfFirstMessage(Message message) {
        if (idHistoryMap.putIfAbsent(String.valueOf(message.getId()), 1) == null) {
            log.info("[Main Consumer2(" + Thread.currentThread().getId() + ")] Message2 arrived! - " + message); // Exactly Once 실행되어야 하는 로직으로 가정
        } else {
            log.info("[Main Consumer2(" + Thread.currentThread().getId() + ")] Duplicate! (" + message.getId() + ")");
        }
    }
}
